...
let new_cases=document.getElementById("status");
...
let critical=document.getElementById('status_ca');
...
fetch("{API-KEY}", requestOptions) //Insert CDC API here
  ...
  new_case.textContent= data.status.toLocaleString();
...
fetch("{API-KEY}", requestOptions) //Insert CDC API
  ...
  for(let i = 0; i<countries_stat.length;i++){
    var circle=L.circle([countries_stat[i].countryInfo.lat, countries_stat[i].countryInfo.long], {
      ...
      fillOpacity: 0.5,
      radius: 200000
    }) .bindPopup("("+countries_stat[i].country+") "+"&#9763;Cases: "+countries_stat[i].cases.toLocaleString()+" | "+" &#9760;Deaths: "+countries_stat[i].deaths.toLocaleString()+" | DATA: "+countries_stat[i].updatedtoLocaleString(),
  )}
).addTo(map);
